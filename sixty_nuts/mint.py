...
# ──────────────────────────────────────────────────────────────────────────────
# Type definitions based on NUT-00 specification
# ──────────────────────────────────────────────────────────────────────────────


class BlindedMessage(TypedDict):
    """Blinded message to be signed by mint (NUT-00 specification).
    
    This is sent from Alice to Bob for minting tokens or for swapping tokens.
    Also called an "output" in the Cashu protocol.
    """
    amount: int  # Value for the requested BlindSignature
    id: str      # Keyset ID from which we expect a signature (hex string)
    B_: str      # Blinded secret message generated by Alice (hex string)


class BlindedSignature(TypedDict):
    """Blind signature from mint (NUT-00 specification).
    
    This is sent from Bob to Alice after minting tokens or after swapping tokens.
    Also called a "promise" in the Cashu protocol.
    """
    amount: int  # Value of the blinded token
    id: str      # Keyset ID of the mint keys that signed the token (hex string)
    C_: str      # Blinded signature on the secret message B_ (hex string)


class Proof(TypedDict):
    """Cashu proof/token (NUT-00 specification).
    
    Also called an "input" and is generated by Alice from a BlindSignature.
    Alice sends Proofs to Bob for melting tokens. Serialized Proofs can
    also be sent from Alice to Carol.
    """
    # Required fields per NUT-00
    id: str          # Keyset ID of the mint public keys that signed the token (hex string)
    amount: int      # Amount of the Proof
    secret: str      # Secret message (utf-8 encoded string, recommend 64-char hex from 32 random bytes)
    C: str           # Unblinded signature on secret (hex string)
